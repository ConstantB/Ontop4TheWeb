...
public class SesameVirtualYelpTest extends TestCase {
	...
	public void test() throws Exception
	{
		...
		try {
			...
			String obdafile = "/home/constant/mappings-ontop/yelp.obda";
			...
			con = repo.getConnection();
			 ...
			 String prefixes = "prefix ex: <http://meraka/moss/exampleBooks.owl#> \n "
						...
						+ "PREFIX yelp: <http://yelp.com/ontology#> \n";
			...
			///query repo
			 try {
				 ...
				 String yelp2 = prefixes + "select distinct ?id ?name  where { \n"
				 		+ "?s yelp:name ?name . \n"
				 		//+ "?s four:hereNow ?now . \n"
				 		+ "?s yelp:hasID ?id . \n"
				 		+ "} ";
				 
				 String yelp3 = prefixes + "select distinct ?id ?name ?rating  where { \n"
					 		+ "?s yelp:name ?name . \n"
					 		+ "?s yelp:rating ?rating . \n"
					 		+ "?s yelp:hasID ?id . \n"
					 		+ "} ";
				 long startTime = System.nanoTime();   
			      TupleQuery tupleQuery = con.prepareTupleQuery(QueryLanguage.SPARQL, yelp3 );
			      
				  TupleQueryResultHandler handler = new SPARQLResultsTSVWriter(System.out);

			      tupleQuery.evaluate(handler);
			      long endTime = System.nanoTime();
				  double eval = (endTime - startTime )/1000000000.0;
				  
				  System.out.println("EVALUATION TIME: " + (endTime - startTime )  + " "  + eval);
